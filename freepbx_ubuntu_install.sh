...
UBUNTU_MIRROR="https://mirrors.ocf.berkeley.edu/ubuntu/"
NPM_MIRROR=""
UBUNTU_OS_VERSION=""

if [ -f /etc/os-release ]; then
    UBUNTU_OS_VERSION=$(grep -oP '(?<=VERSION_CODENAME=).*' /etc/os-release)
fi
...
setup_repositories() {
	...
	if [ -z "$noaac" ]; then
	     ...
	     REPO_LINE="deb $UBUNTU_MIRROR noble main restricted universe multiverse"
